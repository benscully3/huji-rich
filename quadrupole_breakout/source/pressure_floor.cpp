...
/*
Prevents pressure and denisty from dropping below a minimum
   ...
   const CacheData& cd) const
{
  ...
  for(size_t i=0;i<N;++i){
    ...
    res[i].stickers = old[i].stickers;
    if(res[i].density<0) // enforces density floor
      {
	res[i].density = 1e-15;
	ofstream m_out; // writes the added mass and where it was added
	...
	res[i].pressure = eos.de2p(res[i].density,
				   energy,
				   res[i].tracers);
      else // enforces pressure floor
	{
	  res[i].pressure = 1e-16;
	  ofstream m_out; // writes added pressure and where it was added
